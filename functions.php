...
function custom_theme_support() {
    ...
    register_nav_menus( array(              //カスタムメニュー（管理画面の外観-メニューで編集できる）
...
function readScript() {
    //googleフォントの読み込み
    //echo '<link rel="preconnect" href="https://fonts.googleapis.com">'; //preconnectタグはCSSファイルのリンクとは異なり、ブラウザが外部リソースへの接続を事前に確立するためのHTMLタグである。wp_enqueue_style() では処理することができない
    //echo '<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>';
    wp_enqueue_style( 'google-fonts', 'https://fonts.googleapis.com/css2?family=Noto+Sans:ital,wght@0,100..900;1,100..900&display=swap', array(), ''); //バージョンパラメータを null に設定することで、WordPressは自動的にクエリパラメータを付けないようにします
    wp_enqueue_style( 'google-fonts', 'https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap', array(), ''); //バージョンパラメータを null に設定することで、WordPressは自動的にクエリパラメータを付けないようにします
    ...
    wp_enqueue_script( 'jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js','',''); //jqueryの本体読み込み
    wp_enqueue_script( 'script', get_theme_file_uri('/js/main.js'), array('jquery'),'1.0.1',); //arrayは依存関係を指定、このjsﾌｧｲﾙは上行のjqueryを使って書かれたという意味
